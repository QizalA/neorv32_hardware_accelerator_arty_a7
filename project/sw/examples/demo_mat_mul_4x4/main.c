...
static void gemm_cfu(const int8_t A[N][N], const int8_t B[N][N], int32_t C[N][N]){
  for(int i=0;i<N;++i){
    uint32_t ap=pack4_s8_lsb(A[i][0],A[i][1],A[i][2],A[i][3]);
    for(int j=0;j<N;++j){
      uint32_t bp=pack4_s8_lsb(B[0][j],B[1][j],B[2][j],B[3][j]);
      C[i][j]=cfu_dp4a((int32_t)ap,(int32_t)bp);
    }
  }
}
static void gemm_cfs(const int8_t A[N][N], const int8_t B[N][N], int32_t C[N][N]){
  static int8_t A_flat[4*LDA]; static int8_t B_flat[128*LDB]; int32_t C4[4][4];
