/*
Copyright 2018 Embedded Microprocessor Benchmark
Licensed under the Apache License, Version 2.0
Original Author: Shay Gal-on

Modified for NEORV32 + CFU hooks + minimal prints (2025)
*/

#include "coremark.h"

#ifndef COREMARK_MINIMAL_PRINT
#define COREMARK_MINIMAL_PRINT 0 /* set to 1 at compile time to trim strings */
#endif

/* === CFS demo forward declaration (implemented in cfs_demo.c) === */
extern int run_cfs_gemm_demo(void);

/* Known CRC tables (unaltered) */
...
MAIN_RETURN_TYPE main(int argc, char *argv[]) {
  ...
  /* --- CFS GEMM demo (keeps CoreMark kernels untouched) ---------------- */
  /* Uses CoreMark's ee_printf/timer but does NOT modify kernels/CRCs.     */
  run_cfs_gemm_demo();
  /* --------------------------------------------------------------------- */
...
#error "Cannot use a static data area with multiple contexts!"
  ...
  for (i = 0; i < MULTITHREAD; i++) {
    ...
    results[i].size = (malloc_override != 0) ? malloc_override : TOTAL_DATA_SIZE;
